...
fn get_argument_or_env(var_name: &str, default_value: Option<&str>) -> String {
    ...
    let arg_value = env::args().find_map(|arg| {
        if arg.to_lowercase().starts_with(&arg_prefix) {
            if let Some(index) = arg.find('=') {
                ...
                env::args()
                    .skip_while(|a| a.to_lowercase() != arg.to_lowercase())
                    .nth(1)
            }
        } else {
            None
    ...
    arg_value.unwrap_or_else(|| {
        env::var(var_name).unwrap_or_else(|_| {
            if let Some(default) = default_value {
                default.to_string()
            } else {
                eprintln!(
                    "Error: {var_name} must be set via command-line argument or environment variable."
                );
                let lower = var_name.to_lowercase();
                eprintln!("Example: `--{lower}=value` or `--{lower} value` or `export {var_name}=value`");
                std::process::exit(1);
            }
        })
    })
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let batch_size: usize =
        get_argument_or_env("BATCH_SIZE", Some(&DEFAULT_BATCH_SIZE.to_string()))
            .parse()
            .unwrap();
    let collector = get_argument_or_env("1090_COLLECTOR", Some("dump1090"));

    // Connecting to a TCP stream
    let stream = TcpStream::connect(format!("{dump1090_host}:{dump1090_port}"))?;
...
async fn send_to_service(
    messages: Vec<SBS1Message>,
    dataset_api_write_token: &str,
    collector: &str,
